...
func (r *apiLogRepository) FindByInvoice(ctx context.Context, invoiceNumber string) ([]entity.APILog, error) {
	query := `
		SELECT id, endpoint, invoice_no, entry_no, method, request_body, response_body, status_code, duration_ms, email, created_at, "from", "to"
		FROM api_logs
	...
	for rows.Next() {
		...
		if err := rows.Scan(&log.ID, &log.Endpoint, &log.InvoiceNo, &log.EntryNo, &log.Method, &log.RequestBody, &log.ResponseBody, &log.StatusCode, &log.Duration, &log.Email, &log.CreatedAt, &log.From, &log.To); err != nil {
